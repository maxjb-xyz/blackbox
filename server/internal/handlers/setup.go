...
func HealthCheckWithBuildInfo(database *gorm.DB, registry *auth.OIDCRegistry, mcpRuntime healthMCPRuntime, version, commit string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := database.Model(&models.Node{}).Select(
			"COUNT(*) AS total, COALESCE(SUM(CASE WHEN status <> ? AND last_seen > ? THEN 1 ELSE 0 END), 0) AS online, COALESCE(SUM(CASE WHEN last_seen <= ? THEN 1 ELSE 0 END), 0) AS stale",
			nodeStatusOffline, cutoff, cutoff,
			...
			resp.Nodes = healthNodeSummary{
				...
				Offline: nodeStats.Total - nodeStats.Online,
